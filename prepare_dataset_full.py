...
from utils import is_dir_path, padding_tensor
...
MASK_DIR = is_dir_path(parser.get('prepare_dataset','MASK_PATH_FULL'))
IMAGE_DIR = is_dir_path(parser.get('prepare_dataset','IMAGE_PATH_FULL'))
CLEAN_DIR_IMAGE = is_dir_path(parser.get('prepare_dataset','CLEAN_PATH_IMAGE_FULL'))
CLEAN_DIR_MASK = is_dir_path(parser.get('prepare_dataset','CLEAN_PATH_MASK_FULL'))
META_DIR = is_dir_path(parser.get('prepare_dataset','META_PATH_FULL'))
...
class MakeDataSet:
    ...
    def prepare_dataset(self):
        ...
        for patient in tqdm(self.IDRI_list):
            ...
            if int(pid[-4:]) in [35, 74, 114, 218, 361, 447, 479, 523, 619, 661, 697, 760, 850]:
                continue
            ...
            if len(nodules_annotation) > 0:
                ...
                assert vol.shape == mask_volume.shape # Ensure the shape are the same
                ...
                if length > 128:
                    ...
                    lung_np_tensor = vol[crop_start:crop_end, :, :]
                    mask_np_tensor = mask_volume[crop_start:crop_end, :, :]
                    ...
                    # padding
                    lung_np_tensor = padding_tensor(vol)
                    mask_np_tensor = padding_tensor(mask_volume)
