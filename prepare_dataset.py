            ...
            if len(nodules_annotation) > 0:
                ...
                for nodule_idx, nodule in enumerate(nodules_annotation):
                    ...
                    for slice in range(vol.shape[2]):
                        ...
                        if slice not in nodule_idxes:
                            lung_np_array = vol[:,:,slice]
                            lung_np_array = ct_normalize(lung_np_array, slope, intercept)
                            lung_mask = np.zeros_like(lung_np_array)
                            
                            nodule_name = "{}_NI{}_slice{}".format(pid[-4:],prefix[nodule_idx],prefix[slice])
                            mask_name = "{}_MA{}_slice{}".format(pid[-4:],prefix[nodule_idx],prefix[slice])
                            ...
                            np.save(patient_image_dir / nodule_name, resize_image(lung_np_array))
                            np.save(patient_mask_dir / mask_name, resize_mask(lung_mask))
                            # continue
                        else:
                            lung_np_array = vol[:,:,slice]
                            lung_np_array = ct_normalize(lung_np_array, slope, intercept)
                            ...
                            nodule_name = "{}_NI{}_slice{}".format(pid[-4:],prefix[nodule_idx],prefix[slice])
                            mask_name = "{}_MA{}_slice{}".format(pid[-4:],prefix[nodule_idx],prefix[slice])
                            meta_list = [pid[-4:],nodule_idx,prefix[slice],nodule_name,mask_name,malignancy,cancer_label,False]
                            ...
                            np.save(patient_image_dir / nodule_name, resize_image(lung_np_array))
                            np.save(patient_mask_dir / mask_name, resize_mask(mask[:,:,mask_index]))
                            mask_index +=1
