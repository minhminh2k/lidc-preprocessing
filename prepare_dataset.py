...
from utils import make_lungmask, normalize_clipped, segment_lung_mask, preprocess_image
...
class MakeDataSet:
    ...
    def prepare_dataset(self):
        ...
        for patient in tqdm(self.IDRI_list):
            ...
            if int(pid[-4:]) in [35, 74, 114, 218, 447, 479, 523, 619, 661, 697, 760, 850]:
                continue
            ...
            # np.save("/data/hpc/dqm/3D-SegClass-Medical/assets/test/volume_12_resample.npy", vol)
            # break
            # segmentation_np_tensor = []
            # for slice in range(vol.shape[0]-1):
            #     print(slice)
            ...
            # np.save("/data/hpc/dqm/3D-SegClass-Medical/assets/test/k_means_62_lung.npy", segmentation_np_tensor)

            # segmented_lungs = segment_lung_mask(vol, False)
            # segmented_lungs_fill = segment_lung_mask(vol, True) # better
            # np.save("/data/hpc/dqm/3D-SegClass-Medical/assets/test/segmented_lung_fill_false.npy", segmented_lungs)
            # np.save("/data/hpc/dqm/3D-SegClass-Medical/assets/test/segmented_lung_fill_true.npy", segmented_lungs_fill)
            
            # np.save("/data/hpc/dqm/3D-SegClass-Medical/assets/test/segmented_lung_fill_false_multiply.npy", segmented_lungs * vol)
            # np.save("/data/hpc/dqm/3D-SegClass-Medical/assets/test/segmented_lung_fill_true_multiply.npy", segmented_lungs_fill * vol)
            ...
            if len(nodules_annotation) > 0:
                ...
                vol = preprocess_image(images=vol)
